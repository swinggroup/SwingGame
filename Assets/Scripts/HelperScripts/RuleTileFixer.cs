...
public class RuleTileFixer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 2: rename the aseprite from ruletile_template -> whatever is appropriate (usually xxxmap_tiles), and re-slice the sprites (this is to rename the individual sprites)
        ...
        Object[] aseprite = AssetDatabase.LoadAllAssetsAtPath("Assets/Sprites/Tiles/icebgmap_ruletile/icebgmap_tiles.aseprite");
        foreach (Object s in aseprite)
        {
            ...
            if (s.name.StartsWith("icebgmap_tiles_"))
            {
        ...
        RuleTile ruleTile = (RuleTile)AssetDatabase.LoadAssetAtPath("Assets/Sprites/Tiles/icebgmap_ruletile/icebgmap_ruletile.asset", typeof(RuleTile));
